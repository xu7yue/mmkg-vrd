...
from mmkg_vrd.maskrcnn_benchmark.modeling.backbone import resnet
from mmkg_vrd.maskrcnn_benchmark.modeling.make_layers import group_norm
from mmkg_vrd.maskrcnn_benchmark.modeling.make_layers import make_fc
from mmkg_vrd.maskrcnn_benchmark.modeling.poolers import Pooler
from .sparse_targets import _get_tensor_from_boxlist, _get_rel_inds


@registry.ROI_RELATION_FEATURE_EXTRACTORS.register(
    "ResNet50Conv5ROIRelationFeatureExtractor")
class ResNet50Conv5ROIFeatureExtractor(nn.Module):
    ...
    def forward(self, x, proposals, proposal_pairs, use_relu=True):
        ...
        return self._union_box_feats(x, proposal_pairs, use_relu), rel_inds


@registry.ROI_RELATION_FEATURE_EXTRACTORS.register(
    "FPN2MLPRelationFeatureExtractor")
class FPN2MLPFeatureExtractor(nn.Module):
    ...
    def _union_box_feats(self, x, proposal_pairs, use_relu=True):
        proposals_union = [proposal_pair.copy_with_union() for proposal_pair in
                           proposal_pairs]
        x = self.pooler(x, proposals_union)
        x = x.view(x.size(0), -1)
